...
io.Fonts->AddFontFromFileTTF(FontName.c_str(),20,&font_cfg, io.Fonts->GetGlyphRangesChineseFull());
    ...
    style->Colors[ImGuiCol_PopupBg] = ImVec4(200, 99, 71, 220);//弹出窗口背景颜色
    style->Colors[ImGuiCol_Border] = ImColor(139, 0, 0, 255); // 整体边框颜色
    //style->Colors[ImGuiCol_BorderShadow] = ImColor(0, 216, 0,255);//边框阴影颜色(内部控件边框颜色)
    style->Colors[ImGuiCol_FrameBg] = ImColor(255, 99, 71, 200);//框架背景颜色(按钮没动的颜色)
     style->Colors[ImGuiCol_FrameBgActive] = ImColor(255, 99, 71, 200);//激活的框架背景颜色
     style->Colors[ImGuiCol_FrameBgHovered] = ImColor(255, 99, 71, 220);//鼠标悬停的框架背景颜色
     style->Colors[ImGuiCol_ScrollbarBg] = ImColor(255, 99, 71, 220);//滚动区域背景颜色
     style->Colors[ImGuiCol_ScrollbarGrab] = ImColor(255, 165, 0, 255);//滚动条抓取颜色
     style->Colors[ImGuiCol_ScrollbarGrabHovered] = ImColor(0, 216, 0,255);//鼠标悬停时的滚动条抓取颜色
     style->Colors[ImGuiCol_ScrollbarGrabActive] = ImColor(255, 175, 5, 255);//激活时的滚动条抓取颜色
     style->Colors[ImGuiCol_CheckMark] = ImColor(255, 175, 5, 255);//选中标记颜色
     style->Colors[ImGuiCol_SliderGrab] = ImColor(255, 69, 0, 255);//滑块抓取颜色
     style->Colors[ImGuiCol_SliderGrabActive] = ImColor(255, 109, 0, 255);//激活时的滑块抓取颜色
     style->Colors[ImGuiCol_Separator] = ImColor(255, 109, 0, 255);//分隔线颜色
     style->Colors[ImGuiCol_SeparatorActive] = ImColor(255, 109, 0, 255);//激活时的分隔线颜色//深灰色
     style->Colors[ImGuiCol_SeparatorHovered] = ImColor(255, 109, 0, 255);//鼠标悬停时的分隔线颜色
     style->Colors[ImGuiCol_Button] = ImColor(255, 109, 0, 255);//正常按钮颜色//酷安绿
     style->Colors[ImGuiCol_ButtonActive] = ImColor(255, 109, 0, 255);//激活的按钮颜色//微信绿
     style->Colors[ImGuiCol_ButtonHovered] = ImColor(255, 109, 0, 255);//鼠标悬停时的按钮颜色
     style->Colors[ImGuiCol_HeaderActive] = ImColor(255, 109, 0, 255);//激活的标题颜色
     style->Colors[ImGuiCol_HeaderHovered] = ImColor(255, 109, 0, 255);//鼠标悬停时的标题颜色
	 style->Colors[ImGuiCol_TextSelectedBg] = ImColor(255, 129, 71, 200);//选中文本背景颜色
	 style->Colors[ImGuiCol_Header] = ImColor(255, 109, 0, 255);//设置下展标题颜色
...
if (ImGui::Button("物资/SDK",ImVec2(130, 60)))//320
{
菜单 = false;
菜单1 = false;
菜单2 = false;
...
if (ImGui::Button("隐藏悬浮",ImVec2(130, 60)))//380
{
音量键 = false;
}*/
ImGui::EndChild();
...
ImGui::Spacing();
ImGui::TextColored(ImColor(255,0,0,225), "当前版本:V10 『MKS MOD』");
ImGui::Spacing();                                      
ImGui::Text("-耗时 %.3fms   当前帧率 %.1fFPS", 1000.0f / io.Framerate, io.Framerate);
ImGui::Spacing();      
if (ImGui::Button("载入UE4『EGL』",{-1,50})){
重启游戏();
}
if (ImGui::Button("载入UE4『Again』",{-1,50})){

}
ImGui::TextColored(ImColor(0, 255, 0, 255), "前设备分辨率为:2340×1080");
ImGui::Text(("写入模块UE4: 0X%lx\n"),UE4);
if (ImGui::Combo("Ui切换", &style_cd, "凯撒\0巴巴托斯\0散兵\0流浪者")) {
                        switch (style_cd) 
                        {
                        case 0:
...
ImGui::Spacing();
ImGui::Combo("绘制种类", (int *) &HZT, "玩家绘制[1]\0" "玩家绘制[2]\0");
...
ImGui::SliderFloat("自瞄距离", &距离, 0.0f, 450.0f);
if (ImGui::Combo("自瞄部位", &style_idx, "头\0胸\0")) {
switch (style_idx) 
{
case 0:
头部 = true;
...
ImGui::PushStyleColor(ImGuiCol_PopupBg, ImVec4(1.0f, 1.0f, 1.0f, 1.0f));//颜色
                        ...
                        ImVec2 basicWindowPos = ImVec2(aimbotWindowPos.x + aimbotWindowSize.x + 15, aimbotWindowPos.y - 11); 
                        ImVec2 basicWindowSize = ImVec2(ImGui::GetWindowWidth() - aimbotWindowSize.x - 20, ImGui::GetWindowHeight());
                        
                        const ImVec2 minWindowSize = ImVec2(460, 46+0);
                        ...
                        ImGui::TextColored(ImColor(255,204,212), "子追配置选择");
                        ImGui::Spacing();//间距
ImGui::TableNextColumn();	
ImGui::Text("子追配置调整 - (Aimbot set)");  
...
break;
                        ...
                        ImGui::TextColored(ImColor(255,0,0,225), "(PN) 内存功能");
ImGui::Spacing();    
ImGui::Checkbox("广角", &上帝视角);
ImGui::SliderFloat("广角调节", &视角, 0.0f, 300.0f);
ImGui::Checkbox("ipad", &平板视角);
ImGui::SameLine();
